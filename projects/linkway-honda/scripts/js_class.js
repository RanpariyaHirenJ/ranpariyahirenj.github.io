...
//******************** AJAX Code for Get Model , Car Color, Job Category, Emi Calculator *******************************

$(document).ready(function() {
		var car_name = $('input[type="hidden"]').val();
		var func  = "get_car_model";
		
		$.ajax({
			method: "POST",
			url: "ajax_functions.php",
			data: { car_model: car_name , "function": func }
		})
		.done(function( msg ) {
			if(msg)
			{
				$("#car_model").empty();
				$("#car_model").append(msg);
			}
		});
		...
		function(data,status){
			...
			{
				$("#car_color").empty();
				$("#car_color").append(data);
			}
		});
		
		var func  = "car_model_interested";
		$.post("ajax_functions.php",
		{
		  	"function": func
		},
		function(data,status){
			if(data)
			{
				$("#car_model_interest").empty();
				$("#car_model_interest").append(data);
			}
		});
		
		var func  = "get_job_category";
		$.post("ajax_functions.php",
		{
			"function": func
		},
		function(data,status){
			...
			{
				$("#job_category").empty();
				$("#job_category").append(data);
			}
		});
	
		var func  = "get_emicalculator_model";
		$.post("ajax_functions.php",
		{
			"function": func
		},
		function(data,status){
			if(data)
			{	
				$("#emi_calc_model").empty();
				$("#emi_calc_model").append(data);
			}
		});
});

//******************** End Of AJAX Code for Get Model , Car Color, Job Category, Emi Calculator *******************************
	...
	$(container).find("input, select, textarea" ).each(function(){
		...
		{				
			...
			case "file":
				if($(this).val() == $(this).attr("placeholder") || $(this).val() == "")
				{
					$('.upload-resumefile').css("border","1px solid red");
					$(this).val('');
					$('#attach_resume').html('Kindly Attach Resume !')
					return_state = false;													
				}
				else
				{
					$('.upload-resumefile').css("border","1px solid green");
				}
			break;
			case "mobile":
				if($(this).val() == $(this).attr("placeholder") || $(this).val() == "")
...
function match_password(pass1, pass2)
{
	if($(pass1).val() != $(pass2).val())
	{
		...
		alert("Kindly reconfirm your Email!");
...
function match_password1(pass1, pass2)
{
	if($(pass1).val() != $(pass2).val())
	{
		$(pass1).val("");
...
function apply_tabination(container,element)
{
	...
	{
		var tabinationtabs = "<select>"	
		var count = 1;
		$(container).children(element).each(function(){			
				var tabtitle = $(this).attr("tabtitle")
				if(count == 1)
				{		
					tabinationtabs += "<option class=\"tabinationtabs active\" show=\""+count+"\" >"+tabtitle+"</option>"			
				}else{
					tabinationtabs += "<option class=\"tabinationtabs\" show=\""+count+"\">"+tabtitle+"</option>"	
				}
				count++;		
			})		
		tabinationtabs += "</select>"
	}
	...
	{
		$(tabcontainer).find("select").change(function(){
			if(isClicked)
			{
				return;	
			}
			...
			$(this).parent().show();
			$(this).closest(container).children(element+":eq("+$(this).find(":selected").attr("show")+")").fadeIn("slow",function(){
				isClicked = false;	
			});		
		})	
	}
