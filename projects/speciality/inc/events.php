...
if(isset($_POST['customer_event']))
{
	if($_POST['customer_event'] == "Submit"){
		...
		else
		{
			...
			$file_obj = new file_manup("../templates/");
			$file_obj->extract_html("speciality-outdoor-catering.htm");
			$body = $file_obj->replace_tags("{name}|{email}|{phone}|{city}","".ucwords(stripslashes($member_name))."|$member_email|".ucfirst(stripslashes($member_subject))."|".ucfirst(stripslashes($member_message))."");
			$subject  = "    ".ucwords($member_name);
			
			//$to = "ritesh.kalptech@gmail.com";
			$to = "outdoor@speciality.co.in";
			send_email($to,'Speciality Restaurants Ltd.,info@speciality.co.in',$subject,$body);
