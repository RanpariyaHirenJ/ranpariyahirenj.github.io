	...
	if($_POST['action'] == "Submit"){
		...
		if($cptcha != 1){
			...
			if($_FILES["income_tax_file"]['name'] != "" || $_FILES["associative_companies"]['name'] != "" || $_FILES["pan_card_scan_copy"]['name'] != "" || $_FILES["gstin_copy"]['name'] != ""){
				...
				$headers = "From: $from";
				$headers .= "\nBCC: info@intermind.in, hiren@intermind.in, ritesh.kalptech@gmail.com";
            ...
            $count=array_search('g-recaptcha-response',$post_keys);
            $new_arr = array_slice($_POST,0,$count,true);
            $post_values = implode("|",array_values($new_arr));
            
			//enquiry send to enquiry management for intermind
			$message = $post_values."|".$filename."|".$filename1."|".$filename2."|".$filename3;
			$myip = $_SERVER['REMOTE_ADDR'];
			$datetime = date("Y-m-d H:i:s");
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL,"https://www.intermind.in/admin/fetch_enquiry.php");
			curl_setopt($ch, CURLOPT_POST, 1);
			curl_setopt($ch, CURLOPT_POSTFIELDS, "user_id=33&project_name=Speciality&enquiry_type=Vendor Register&enquiry_details=$message&datetime=$datetime&cust_ip=$myip");
