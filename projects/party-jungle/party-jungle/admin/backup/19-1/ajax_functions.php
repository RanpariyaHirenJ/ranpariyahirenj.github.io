...
switch($function)
{
    ...
    <div style=\"padding:5px 20px; \">
		 ...
		 while($value = mysqli_fetch_array($result))
		 {
		  ...
		  // modify form
		  $data .="<iframe id=\"modify_info_save".$value['order_detail_id']."\" name=\"modify_info_save".$value['order_detail_id']."\" style=\"display:none;\"></iframe>
		  <style type=\"text/css\">
		  	.disp
			{
				display:none;
		  ...
		  <form action=\"postdata_order.php?action=Continue\" style=\"display:none;\" method=\"post\" id=\"modify_item_form".$frmcount."\" target=\"modify_info_save".$value['order_detail_id']."\">
            ...
            <td width=\"10%\"><table width=\"100%\" style=\"text-align:center;\">
             ...
             ";
				   	if($value['item_photo'] != "")
						...
						{ 
					$data .= "<img src=\"../item-photos/no-photo.png\" width=\"100\" height=\"100\" />";
                    	}
					$data .="</td>
                </tr>
                <tr>
                  <td><label style=\"font-size:14px;\"><strong><u>".ucwords($value['item_name'])."</u></strong>
                      <input name=\"itmid\" id=\"itmid\" type=\"checkbox\" style=\"position:relative; top:3px;\" value=".$value['item_id'].">
                    </label></td>
                </tr>
                <tr>
                     <td><label><strong><u>".ucwords($value['item_name'])."</u></strong></label></td>
                        </tr>
              			</table></td>
            <td width=\"90%\"><table width=\"100%\">
                ...
                <tr>
                  <td><label style=\"display:block;\">Our Cost</label>
				  	<input name=\"order_id\"  type=\"text\" value=".$value['order_id'].">
                    <input name=\"our_cost\"  type=\"text\" value=".$value['our_cost'].">
					<input name=\"item_id\"  type=\"hidden\" value=".$value['item_id']."></td>
                    <td><label style=\"display:block;\">Customers Cost</label>
                    <input name=\"customer_cost\" type=\"text\" value=".$value['client_cost'].">";
                  $data .= "<td><label style=\"display:block;\">Quantity</label>
                    <input name=\"quantity\"  type=\"text\" value=".$value['quantity']."></td>
                </tr>
                <tr>";
        				   $tdcount = 1;
                            ...
                            while($value1 = mysqli_fetch_array($result1))
                            {
								...
								$data .= "<td>".ucwords($value1['property_name'])."<br>";
								$data .="<select name=".$value1['property_id'].">";
                                 $sql2 = "SELECT * FROM craft_items_properties_value WHERE property_id = $property_id";
                                    $result2 = $db_object->execute_query($sql2);
                                    while($value2 = mysqli_fetch_array($result2))
                                    {
					$data .="<option value=".$value2['property_value_id']." ".$db_object->return_compare($value2['property_value_id'],$order_property_id,"selected=\"selected\"","").">".ucwords($value2['property_value_name'])."</option>";
                                    }
                    $data .="</select></td>";
                                if($tdcount%3 == 0)
                                {
                ...
                $data .="</tr>
                <tr>
                  <td colspan=\"3\"><textarea cols=\"72\" rows=\"2\" placeholder=\"Enter the customer products notes here\" name=\"item_note\">".$note."</textarea></td>
                </tr>
              </table></td>
          </tr>
        </table>
	 ...
	 <input type=\"submit\" name=\"action\" value=\"Modify\" style=\"margin-right:10px;\" onClick=\"modify_order_form(".$frmcount.")\" class=\"action-button\">
	 <input type=\"button\" name=\"action\" value=\"Done\" style=\"margin-right:10px;padding:4px 25px\" onClick=\"submit_order_form(".$frmcount.")\" class=\"action-button\"></h2>";
	break;
