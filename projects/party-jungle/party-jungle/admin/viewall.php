  ...
  <div class="container order">
	...
	while($value = mysqli_fetch_array($result))
	{
 	 ...
 	 ?>
            ...
            <iframe id="info_save<?php echo $value['order_detail_id']; ?>" name="info_save<?php echo $value['order_detail_id']; ?>" style="display:none;"></iframe>
            <form action="postdata_order.php?action=Continue" style="display:none;" method="post" id="modify_item_form<?php echo $frmcount; ?>" target="info_save<?php echo $value['order_detail_id']; ?>">
        <table width="76%" border="0" cellpadding="0" align="center" cellspacing="0" class="product-list" >
          <tr>
            ...
            <?php 	   	if($value['item_photo'] != "")
						{
							echo $photo_obj->imageResize(get_thumbnail($value['item_photo']),100);
						}
						else
						{ ?>
                    ...
                    <?php } ?>
				<tr>
                  <td><label style="font-size:14px;"><strong><u><?php echo ucwords($value['item_name']); ?></u></strong>
                      <input name="itmid" id="itmid" type="checkbox" style="position:relative; top:3px;" value="<?php $value['item_id'];?>">
                    </label></td>
                </tr>
                <tr>
                     <td><label><strong><u><?php echo ucwords($value['item_name']);?>"</u></strong></label></td>
                        </table></td></tr>
            <td width="90%"><table width="100%">
                ...
                <tr>
                  <td><label style="display:block;">Our Cost</label>
				  	<input name="order_id"  type="text" value="<?php echo $value['order_id']; ?>">
                    <input name="our_cost"  type="text" value="<?php echo $value['our_cost']; ?>">
					<input name="item_id"  type="hidden" value="<?php echo $value['item_id']; ?>"></td>
                    <td><label style="display:block;">Customers Cost</label>
                    <input name="customer_cost" type="text" value="<?php echo $value['client_cost'];?>">
                 	<td><label style="display:block;">Quantity</label>
                    <input name="quantity"  type="text" value="<?php echo $value['quantity'];?>"></td>
                </tr>
                <tr> <?php 
        				   $tdcount = 1;
                            $sql1 = "SELECT * FROM craft_items_properties WHERE item_id =".$value['item_id']." AND property_status = 0" ;
                            $result1 = $db_object->execute_query($sql1);
                            while($value1 = mysqli_fetch_array($result1))
                            {
								$property_id = $value1['property_id'];
								$sql_item = "SELECT order_detail_id, item_id FROM order_detail WHERE order_id =".$value['order_id']." AND item_id =".$value['item_id'];
						  		$result_select = $db_object->return_query($sql_item);
								
								$sql2 ="SELECT property_value_id FROM order_property WHERE property_id = ".$property_id." AND order_detail_id=".$result_select['order_detail_id'];
								$result2 = $db_object->return_query($sql2);
								$order_property_id = $result2['property_value_id'];
								?>
								<td><?php echo ucwords($value1['property_name']);?><br>
								<select name="<?php echo $value1['property_id']; ?>">
                                <?php $sql2 = "SELECT * FROM craft_items_properties_value WHERE property_id = $property_id";
                                    $result2 = $db_object->execute_query($sql2);
                                    while($value2 = mysqli_fetch_array($result2))
                                    { ?>
                                	<option value="<?php echo $value2['property_value_id']; ?>" <?php echo $db_object->return_compare($value2['property_value_id'],$order_property_id,"selected=\"selected\"","") ?>><?php echo ucwords($value2['property_value_name']); ?></option>
								    <?php } ?>
                ...
                <tr>
                  <td colspan="3"><textarea cols="72" rows="2" placeholder="Enter the customer products notes here" name="item_note"><?php echo $note; ?></textarea></td>
                </tr>
              </table></td>
          </tr>
        </table>
...
<div class="footer-buttons select-contents" style="width:100%;">
  <div style="width:50%;float:left;text-align:left;margin-top:15px;margin-left:100px;">
    ...
    &nbsp;
    <input type="button" value="Back to Calendar" class="action-button" onClick="window.location='calendar.php';">
    <?php if($frmcount != 0){ ?>
     &nbsp;
    <input type="submit" value="Modify" class="action-button" onClick="modify_order_form('<?php echo $frmcount; ?>')">
    <?php } ?>
  </div>
