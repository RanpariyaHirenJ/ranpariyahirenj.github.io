...
$client_email    = $_SESSION['client']['clent_email'];
$delivery_date  = $_SESSION['client']['delivery_date'];
     ...
     <?php
      ...
      <div class="footer-buttons select-contents" style="padding:15px 0 5px 0;">
			...
			function confirm_order()
			{
				...
				if (status2) {
        		<?php if(!isset($_SESSION['form_action'])) { ?>
				//alert('order-summary.php?action=Save');
        		location.assign('order-summary.php?action=Save');
				<?php } else { ?>
				//alert('order-summary.php?action=Modify');
				location.assign('order-summary.php?action=Modify');	
    			<?php } ?>
				}
