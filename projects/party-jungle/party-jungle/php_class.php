		...
		{				
		 ...
		 for($i=($this->start - 5); $i < ($this->start + 5);$i++)
		 {
				if($i <> $this->start)
				{
					...
					{					
						$paging = $paging."<a href=".$this->page."?start=".$i."&".$this->params." style=\"background-color:#CCC; padding: 2px 5px; border-radius:20px;margin:2px\">".$i."</a>";
					}
				}	
				else 
				{ 
					$paging = $paging. "<span class=\"current\" style=\"font-size:12px; color:#FFF; background-color:#000 !important; padding: 2px 5px; border-radius:20px;margin:2px\">".$this->start."</span>";
				} 
		 }
...
class img_manup
{
	...
	function imageResize($filename, $area, $titletext="", $atltext="",$classtext="") 
	{
		...
		return "<img src='$imagepath' width=\"".$width."\" height=\"".$height."\" border=\"0\" title=\"".$titletext."\" alt=\"".$atltext."\" class=\"".$classtext." disp\"/>"; 
	} 
	...
	function createThumbs($filename, $thumbnailname, $imgratio, $text="") 
	{
		...
		if($image_result[0] == "True")
		{
			...
			$base_img = imagecreatetruecolor($imgratio, $imgratio );
			imagealphablending($base_img, false);
			imagesavealpha($base_img,true);
			$transparent = imagecolorallocatealpha($base_img, 255, 255, 255,127);
			imagefilledrectangle($base_img, 0, 0, $imgratio, $imgratio, $transparent);
				
			imagecopyresampled($base_img,$tmp_img,($imgratio - $new_width)/2,($imgratio - $new_height)/2,0,0,$new_width,$new_height,$new_width,$new_height);
			...
			{
				case ".jpg":
					imagejpeg($base_img, "$pathToThumbs".$image_result[1]);
				break;
				...
				case ".gif":
					imagegif($base_img, "$pathToThumbs".$image_result[1]);
				break;
				
				case ".png":
					imagepng($base_img, "$pathToThumbs".$image_result[1]);
				break;			
			}
			imagedestroy($img);			
		}
