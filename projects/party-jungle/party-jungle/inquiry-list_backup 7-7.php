	...
	switch($_REQUEST['action'])
	{
		...
		case"Place Inquiry":
			...
			foreach($_SESSION['wishlist'] as $key => $val)
			{
				...
				if($val[2] != "")
				{		 
					$data .= "<strong>Message :</strong> ".ucfirst($val[2])."<br>";	
				}
			...
			$from = $email;
			$to = $email;
			send_email($to,$from,"Product inquiry from party jungle",$data);
			unset($_SESSION['wishlist']);
...
<div class="wrapper">
  ...
  <section class="content-area">
    <section class="block party-price"></section>
    <section class="block extra-offers">
      <div class="container">
...
if(isset($_SESSION['wishlist']))
  ...
  ?>
              <li>
                <div class="inquiry-delete-btn"><a href="inquiry-list.php?action=delete&item=<?php echo $key ?>"><img src="images/close-icon.png" width="43" height="43" /></a></div>
                <?php 
		if($result['item_photo'] != "")
		{
			?>
                ...
                <?php
			echo $photo_obj->imageResize(get_thumbnail($result['item_photo']),225);
		}
		else
		{ 
	?>
        ...
        foreach($val[1] as $keys => $vals)
		{
			...
			?>
                   	...
                   	<div style="width:150px;font-size:20px;"> <?php echo ucwords($result1['property_name']) ?>
                    <div style="float:right;width:70px;">:<?php echo ucwords($result2['property_value_name']) ?></div>
                  </div>
			...
			?>
                    ...
                    <textarea style="height:auto; font-size:12px; padding:4px !important"><?php echo ucfirst($val[2]); ?></textarea>
                    <div>&nbsp;</div>
                    <?php
			}
        }
