...
// *** Refer friend Form Validation Start**//
function validate_form(container)
{
	...
	return return_state;
	/*if(return_state){
		document.referfriend.method ="post";
		document.referfriend.action ="inc/mail.php";
		document.referfriend.submit();
	}*/
}

// *** Refer friend Form Validation End**//
	...
	$(container).find("input, select, textarea").each(function(){
		...
		{				
			...
			case "file":
				if($(this).val() == $(this).attr("placeholder") || $(this).val() == "")
				{
					$(this).prev('.file-upload > span').css("border","1px solid red");
					$(this).val('');
					return_state = false;													
				}
				else
				{
					$(this).prev('.file-upload > span').css("border","1px solid green");
				}
			break;
			case "mobile":
				if($(this).val() == $(this).attr("placeholder") || $(this).val() == "")
