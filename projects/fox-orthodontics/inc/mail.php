...
if(isset($_REQUEST['action']))
{
	if($_REQUEST['action'] == "SUBMIT")
	{
		...
		$message = ucfirst(stripslashes($name))."|$email";
		$myip = $_SERVER['REMOTE_ADDR'];
		$datetime = date("Y-m-d H:i:s");
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL,"http://www.intermind.in/admin/fetch_enquiry.php");
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, "user_id=8&project_name=Fox Orthodontics&enquiry_type=Newsletter&enquiry_details=$message&datetime=$datetime&cust_ip=$myip");
	...
	if($_REQUEST['action'] == "Send")
	{
		...
		$fromm 					= $db_object->sanatize_string($_REQUEST['from']);
		...
		if (strpos($fromm, 'Dr.') !== false) {
		   $fromm = str_replace('Dr.', '', $fromm); 
		}
		else if (strpos($fromm, 'Dr') !== false) {
		   $fromm = str_replace('Dr', '', $fromm);  
		}
		$fromm = trim($fromm);
		...
		if(trim($_REQUEST["captcha_code"]) != "" && isset($_REQUEST["captcha_code"]))
		{
			...
			if($valid != true) 
			{
				echo "<script>location.href = '".$url_name[0]."?error=The code is invalid.'</script>";
				die();
			} 
		}
		else
		...
		$subject1 = "You have been referred for an orthodontic treatment by Dr ".ucwords($fromm);
		$body1 = "Dear ".ucwords($sendto).",<br/><br/>
				Dr ".ucwords($fromm)." has referred you to Fox Orthodontics for an orthodontic assessment.<br/><br/>
		...
		//* Mail Sent to admin  */
		$subject = "Message from Dr ".ucwords($fromm);
		$body = "Dr Rajiv,<br/><br/>";
		$body .= "Thank you for seeing ".ucwords($sendto).". ".ucwords($sendto)." presents \"".stripslashes($note)."\".<br/><br/>
				Referrer's contact no. : $referrers_contact_no<br />
				Patient's contact no. : $patients_contact_no<br />
                                Patient's Email : <a href='mailto:$email_to'>".$email_to."</a><br /><br />
				Warm regards,<br/>";
		$body .= "Dr ".ucwords($fromm);
		...
		//enquiry send to enquiry management for intermind
		$message = ucfirst(stripslashes($fromm))."|$from_email|$referrers_contact_no|$sendto|$email_to|$patients_contact_no|".stripslashes($note)."|http://www.foxorthodontics.com.au/uploads/".$filename;
		$myip = $_SERVER['REMOTE_ADDR'];
		$datetime = date("Y-m-d H:i:s");
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL,"http://www.intermind.in/admin/fetch_enquiry.php");
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, "user_id=8&project_name=Fox Orthodontics&enquiry_type=Online Referral&enquiry_details=$message&datetime=$datetime&cust_ip=$myip");
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		$server_output = curl_exec ($ch);
		curl_close ($ch);
...
if(isset($_REQUEST['apppointment']))
{
	if($_REQUEST['apppointment'] == "SUBMIT")
	{
		...
		//enquiry send to enquiry management for intermind
		$message = ucfirst(stripslashes($member_name))."|$appoint_date|$moblie|$email|".stripslashes($message);
		$myip = $_SERVER['REMOTE_ADDR'];
		$datetime = date("Y-m-d H:i:s");
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL,"http://www.intermind.in/admin/fetch_enquiry.php");
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, "user_id=8&project_name=Fox Orthodontics&enquiry_type=Appointment Page&enquiry_details=$message&datetime=$datetime&cust_ip=$myip");
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		$server_output = curl_exec ($ch);
		curl_close ($ch);
