...
RangeSlider.prototype = {
    main: function() {
        ...
        // Update the bar price
        this.setBar(this.price);
    },
    construct: function() {
    ...
    events: function() {
        ...
        this.$el.on("touchstart mousedown", function(e) {
            ...
            var price = _this.getprice(_this.lastCoordinates);
            _this.price = price;

            if (typeof(_this.onDown) === "function") {
                _this.onDown(e, price*100);
            }
        });
        ...
        var onLeave = function(e) {
            if (_this.active === true) {
                ...
                if (typeof(_this.onUp) === "function") {
                    if (typeof(_this.onMove) === "function") {
                        _this.onMove(e, price*100);
                    }
                    _this.onUp(e, price*100);
                }
            }
        };
    ...
    fixBar: function(coordinates) {
        var price = this.getprice(coordinates);

        // Set the bar
        this.setBar(price);
    },
    getprice: function(coordinates) {
        ...
        // price
        var price = width / this.$el.width();

        // Return the price
        return price;
    },
    setBar: function(price) {
        if (price < 0 || price > 1) {
           throw new Error("RangeSlider: setBar expects a price between 0-1, ", price, "given");
        }

        // Determine how far left 
        var width = this.$el.width() * price;
        var left = width;

        // Apply calculated values
        this.$rangeinner.css("width", (price*100)+"%");
        this.$rangeselector.css("left", (price*100)+"%");
    },
    ...
    setprice: function(price) {
        if (price < 0 || price > 100) {
            throw new Error("RangeSlider: setprice expects a price between 0-100, ", price, "given");
        }
        this.setBar(price/100);
    }
}
