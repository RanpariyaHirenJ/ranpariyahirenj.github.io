    ...
    $.fn.imageLightbox = function (opts) {
        var options = $.extend({
            ...
            previousTargetDefault: function () {
                ...
                console.log($(target).attr("data-text"))

                $('#caption').remove();

                setTimeout(function () {
                    $('<div id="caption">' + $(target).attr("data-text") + '</div>').appendTo('.image-wrap');
                }, 500)
            ...
            loadImage = function (direction) {
                ...
                setTimeout(function () {
                    ...
                    image = $('<img ' + options.selector + ' />').attr('src', imgPath).load(function () {
                        
                        image.appendTo('.image-wrap');
                        setImage();
                        var params = {
                            'opacity': 1
                        };
                        image.css('opacity', 0);
                        if (isCssTransitionSupport) {
                            cssTransitionTranslateX(image.parent(), -100 * direction + 'px', 0);
                            setTimeout(function () {
                                cssTransitionTranslateX(image.parent(), 0 + 'px', options.animationSpeed / 1000);
                            }, 50);
                        } else {
                            var imagePosLeft = parseInt(image.parent().css('left'));
                            params.left = imagePosLeft + 'px';
                            image.parent().css('left', imagePosLeft - 100 * direction + 'px');
                        }
                        image.animate(params, options.animationSpeed, function () {
                            inProgress = false;
