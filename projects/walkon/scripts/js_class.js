...
function multiple_form_validate_jquery(container)

{
	...
	$(container).find("input, select, textarea, checkbox" ).each(function(){
		...
		// Check WhichNumber of button clicked 
		buttonval = $(document.activeElement).attr('id');
		no = $.trim(buttonval.match(/\d+/));
		number = parseInt(no) + 1;
		// Check Which Number of button clicked 
		switch($(this).attr("validation"))
		{				
			case "text":
				if($(this).val() == $(this).attr("placeholder") || $(this).val() == "")
			...
			case "select":
				if($(this).val() == $(this).attr("placeholder") || $(this).val() == "")
				{
					$("#input-type-gender"+no).css("background-image","linear-gradient(red, red), linear-gradient(red, red)");
					$("#input-type-location"+no).css("background-image","linear-gradient(red, red), linear-gradient(red, red)");
					//$(".input-type-select").css("background-image","linear-gradient(red, red), linear-gradient(red, red)");
					$(this).val('');
					//$(this).attr("placeholder", ""+title+" cannot be blank!")
					return_state = false;													
				...
				{
					//$(this).css("border-bottom","1px solid green");
					$("#input-type-gender"+no).css("background-image","linear-gradient(green, green), linear-gradient(green, green)");
					$("#input-type-location"+no).css("background-image","linear-gradient(red, red), linear-gradient(red, red)");
					//$(".input-type-select").css("background-image","linear-gradient(green, green), linear-gradient(green, green)");
				}
			break;
			...
			case "email":
				...
				{
					/*var email_address = $('#email'+no).val();
					var confirm_email = $('#confirm_email'+no).val();
					if(email_address != confirm_email)
					{
						alert('Kindly check your email.');
						$('#email'+no).focus();
			...
			case "number":
				...
				{
					//$(this).css("border-bottom","1px solid green");
					/*var mobile = $('#mobile_no'+no).val();
					var confirm_mobile = $('#confirmmobile_no'+no).val();
					if(mobile != confirm_mobile)
					{
						alert('Kindly check your mobile number.');
						$('#mobile_no'+no).focus();
			...
			case "check":
				...
				{
					var okay = false;
					if($('#check-indi5:checked').length > 0)
					{
						okay = true;
						break;
					}
