$(window).load(function () {  
	setTimeout(function() {
		//alert("hello")
		$(".product-banner .slider-row").css("display", "block");
	}, 300);
});
...
$(".slick-slide.slick-current.slick-active").removeClass("slick-active");
setInterval(function(){
	  if ($(".slider-row.slick-slide.slick-current").hasClass('active'))
		{
		}
		else
		{
			$(".slider-row.slick-slide.slick-current").addClass('active');
		}
   }, 500);
})
...
//  $('body').append('<div id="empDtlError" class="overlay-box"><div class="seisson-message error"><span></span> Overlay id is not defined </div></div>');
  ...
  $('.accordion dl dt').click(function () {
    ...
    /*accordion start*/
    /*Accordian start*/
  $(".headingBlocks").find('h2').css("cursor","pointer")

      //$('.block-content').hide();
      //$('.first-block').show();
      //$(".headingBlocks").find('h2').each(function () {
      //    $(this).click(function () {
      //        $('.block-content').hide();
      //        $(this).parent().find('.block-content').show()
      //    })
    //})

      $('.titlingrow').addClass('faq-hide');
      $('.titlingrow:eq(0)').removeClass('faq-hide');
      $('.titlingrow:eq(0)').addClass('faq-show');
      //$('.first-block').removeClass('faq-hide');
      //$('.first-block').addClass('faq-show');
      $(".headingBlocks").find('h2').each(function () {
          $(this).click(function () {
              $('.titlingrow').removeClass('faq-show');
              $('.titlingrow').addClass('faq-hide');
              $(this).parent().removeClass('faq-hide');
              $(this).parent().addClass('faq-show');
          })
      })
...
(function() {
  ...
  function toggleHandler(toggle) {
	...
	$(document).on('click','.ss-hovercontent h2',function(){
		if($(window).width() < 768)
			{
				var Sttus_mob=$(this).next('p').css('display');
				if(Sttus_mob=='none')
				{
					$(this).next('p').slideDown();
					$(this).parents('.ss-hovercontent').addClass('mobilehoverbx');
				}
				else
				{
					$(this).next('p').slideUp();
					$(this).parents('.ss-hovercontent').removeClass('mobilehoverbx');
					}
			}
		});
		$(document).on('click','.hasDropDown',function(){
			$('.submenu').removeClass('flipInY');
...
if ($(window).width() < 1024) {
	var stickyNavTop = $('.project-tab-cntr').offset().top
		var stickyNav = function(){
			var scrollTop = $(window).scrollTop();
			if (scrollTop > (78)) { 
					$('.project-tab-cntr').addClass('fixBtn');
			} else {
				
					$('.project-tab-cntr').removeClass('fixBtn');
			}
