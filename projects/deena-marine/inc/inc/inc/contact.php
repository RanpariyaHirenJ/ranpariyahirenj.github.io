...
if(isset($_POST['action']))
{
	if($_POST['action'] == "Submit")
	{
		...
		else
		{
    		...
    		$to = "arif@techflowengg.com";
    		if($trimmedip != "184.154.36" && $member_name != "1" && $member_email != "1"){
        		mail($to, $subject, $body, $headers);
        		
        		//enquiry send to enquiry management for intermind
        		$message = "Techflow|".ucwords(stripslashes($member_name))."|$member_email|$member_mobile|".ucfirst(stripslashes($country_name))."|".ucfirst(stripslashes($interested_in))."|".ucfirst(stripslashes($message));
	...
	if($_POST['action_enquiry'] == "Submit")
	{
		...
		else
		{
    		...
    		if($trimmedip != "184.154.36" && $member_name != "1" && $member_email != "1"){
    		    ...
    		    //enquiry send to enquiry management for intermind
    		    $message = "Techflow|".ucwords(stripslashes($member_name))."|$member_email|$member_mobile|".ucfirst(stripslashes($country_name))."|".ucfirst(stripslashes($interested_in))."|".ucfirst(stripslashes($subject))."|".ucfirst(stripslashes($message));
