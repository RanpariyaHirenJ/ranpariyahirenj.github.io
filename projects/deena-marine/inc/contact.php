...
if(isset($_POST['action']))
{
	if($_POST['action'] == "Submit Now")
	{
		...
		else
		{
    		...
    		$enquiry_type 	= $db_object->sanatize_string($_POST['enquiry_type']);
    		$country  = $db_object->sanatize_string($_POST['country']);
    		$message 		= $db_object->sanatize_string($_POST['message']);
			
			if($enquiry_type == ""){ $enquiry_type = "Contact Page"; }
    		...
    		$body = $file_obj->replace_tags("{name}|{email}|{phone}|{product}|{enquiry_type}|{country}|{message}","".ucwords(stripslashes($member_name))."|$member_email|$member_mobile|".ucfirst(stripslashes($product))."|".ucfirst(stripslashes($enquiry_type))."|".ucfirst(stripslashes($country))."|".ucfirst(stripslashes($message))."");
    		
    		$subject  = "You have a new enquiry from ".ucwords($member_name);
    		$headers = "MIME-Version: 1.0"."\r\n";
    		$headers .= "Content-type:text/html;charset=UTF-8"."\r\n";
    		$headers .= 'From: Deena <info@deenamarine.com>'."\r\n";
