...
$(document).ready(function() {
	$('#webSocialcopylink').click(function(){
		var message = $(location).attr('href');
	...
	$('#webSocialwhatsapp').click(function(){
		var page_url = $(location).attr('href');
		var whattup_url = "whatsapp://send?text="+page_url;
		window.open(whattup_url,'_blank');
	});
	
	$('#webSocialTwitter').click(function(){
		var page_url = $(location).attr('href');
		var socialtwitter = "https://twitter.com/intent/tweet?text="+page_url;
		window.open(socialtwitter,'_blank');
	});
	
	$('#webSocialfacebook').click(function(){
		var page_url = $(location).attr('href');
		var socialfacebook = "https://www.facebook.com/sharer/sharer.php?u="+page_url;
		window.open(socialfacebook,'_blank');
	});
	
	$('#webSocialLinkedin').click(function(){
		var page_url = $(location).attr('href');
		var sociallinkedin = "http://www.linkedin.com/shareArticle?mini=true&amp;url="+page_url;
		window.open(sociallinkedin,'_blank');
	});
});	
		...
		if(captcha != "")
		{
			...
			function(data,status){
				...
				else
				{
					...
					$(".Captchabox-lgn").empty();
					$(".Captchabox-lgn").append(data);
...
function validate_enquiry_form(form_name)
    {
		var return_state = true;
		$('#'+form_name).find("input, select, textarea, checkbox" ).each(function(){
		var title = $(this).attr("title");			
		...
		if(captcha != "")
		{
			...
			function(data,status){
				...
				else
				{
					...
					$("input[name='captchacode']").prev().addClass('active');
					$("input[name='captchacode']").attr("placeholder", "The code is invalid.")
					$("input[name='captchacode']").css("border","1px solid red");
					return false;
				}
			});
		}
...
function validate_buy_form(form_name)
    {
		var return_state = true;
		$('#'+form_name).find("input, select, textarea, checkbox" ).each(function(){
		var title = $(this).attr("title");			
...
function validate_enquiryform(form_name)
    {
		var return_state = true;
		$('#'+form_name).find("input, select, textarea, checkbox" ).each(function(){
		var title = $(this).attr("title");			
		...
		if(captcha != "")
		{
			...
			function(data,status){
				...
				else
				{
					...
					$("#captcha_code").attr("placeholder", "The code is invalid.")
...
function form_validate_jquery(container)
{
	...
	$(container).find("input, select, textarea, checkbox" ).each(function(){
		...
		{				
			...
			break;
			case "check":
				if(!$('#above18years').prop("checked"))
				{
					alert('Kindly check "I am above 18 years of age and I agree to pay by cash"');
					return_state = false;
				}
				else 
				{
					r = confirm('Are you sure you want place a Cash On Delivery order ?');
					if(r == true){ 
						return_state = true; 
						$('#conord').hide();
						$('#processing1').show();
...
if(param[0] == "enquiry")
{
	...
	/*$("body").append("<div id='dialogfirm' title='Success' style='display:none;'><p><span class='ui-icon ui-icon-alert' style='float:left; margin:12px 12px 20px 0;'></span>Your Enquiry has been Submitted Successfully. Our team will contact you shortly.</p></div>").css("overflow","hidden");
	$( "#dialogfirm" ).dialog({
      resizable: false,
      height: "auto",
      width: 400,
