...
if(isset($_REQUEST['action']))
{
	if($_REQUEST['action'] == "Submit")
	{
		...
		$smessage = "https://www.intermind.in/clients/finlinea/html/inc/curl_mail.php?action=sendmail_contact&member_name=".stripslashes(urlencode($member_name))."&member_email=$member_email&member_mobile=$member_mobile&comments=".stripslashes(urlencode($comments))."";
...
else if(isset($_REQUEST['action_buy']))
{
	if($_REQUEST['action_buy'] == "Submit")
	{
		...
		$check_quantity = 0;
		if($aloe_vera_name != "" && $aloe_vera_price != "" && $aloe_vera_qty != "" && $aloevera_totalprice != "") { $check_quantity = $check_quantity + $aloe_vera_qty; }
		if($karela_name != "" && $karela_price != "" && $karela_qty != "" && $karela_totalprice != "") { $check_quantity = $check_quantity + $karela_qty; }
		if($jamun_name != "" && $jamun_price != "" && $jamun_qty != "" && $jamun_totalprice != "") { $check_quantity = $check_quantity + $jamun_qty; }
		
		$check_discount = 0;
		if($check_quantity > 1 && $check_quantity < 4){
			$check_discount = sprintf('%0.2f',$grand_total_val * ((10) / 100));
		}
		else if($check_quantity >= 4){
			$check_discount = sprintf('%0.2f',$grand_total_val * ((15) / 100));
		}
		else
		{
			$check_discount = 0;
		...
		if($grand_total_val_check == $grand_total_val && $check_discount == $discount_val && $check_payble_amount == $payable_amount){
			$smessage = "https://www.intermind.in/clients/finlinea/html/inc/curl_mail.php?action=sendmail_enquiry&member_name=".urlencode($member_name)."&member_email=$member_email&member_mobile=$member_mobile&member_phone=$member_phone&member_address=".urlencode($member_address)."&member_message=".urlencode($member_message)."&aloe_vera_name=$aloe_vera_name&aloe_vera_price=$aloe_vera_price&aloe_vera_qty=$aloe_vera_qty&aloe_vera_totalprice=$aloevera_totalprice&karela_name=$karela_name&karela_price=$karela_price&karela_qty=$karela_qty&karela_totalprice=$karela_totalprice&jamun_name=$jamun_name&jamun_price=$jamun_price&jamun_qty=$jamun_qty&jamun_totalprice=$jamun_totalprice&grand_total_val=$grand_total_val&discount_val=$discount_val&payable_amount=$payable_amount";
			$url = str_replace(" ", '%20', $smessage);
			$ch = curl_init($url);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
