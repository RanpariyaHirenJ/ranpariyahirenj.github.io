...
if(isset($_POST['action'])){
	if($_POST['action'] == "APPLY NOW"){
		...
		if($cptcha != 1){
			...
			$body = $file_obj->replace_tags("{name}|{apply_for}|{location}|{technical_skill}|{qualification}|{experience}","".ucwords(stripslashes($member_name))."|".ucfirst(stripslashes($apply_for))."|".ucfirst(stripslashes($location))."|".ucfirst(stripslashes($technical_skills))."|".stripslashes($qualification)."|".stripslashes($experience)."");
