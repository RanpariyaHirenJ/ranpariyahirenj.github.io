...
$(document).on('click',".get-started a", function(){
	...
	if(urlname == "https://www.vsynergizeoutsourcing.com/"){ sessionStorage.setItem('leadpage','index'); }
	else { 
		var filename = url.substring(url.lastIndexOf('/')+1); filename = filename.substr(0, filename.lastIndexOf('.'));
		var page_name = filename.replace(/\-/g, " "); sessionStorage.setItem('leadpage',page_name); 
	}
});
