...
class img_manup
{
	...
	function createThumbs($filename, $thumbnailname, $imgratio, $dimensions, $text="") 
	{
		$image_result = $this->validate_image(strtolower($filename));
		...
		if($image_result[0] == "True")
		{
			...
			{
				case ".jpg":
					imagejpeg($tmp_img, "$pathToThumbs".$image_result[1],95);
				break;
				...
				case ".png":
					imagepng($tmp_img, "$pathToThumbs".$image_result[1]);
				break;			
			}
			imagedestroy($img);	
...
class file_manup
{
	var $folderpath;
	var $content;
	var $mime_type = array('pdf' => 'application/pdf', 'apk' => 'application/vnd.android.package-archive', 'ios' => 'application/vnd.apple.pkpass', 'xls' => 'application/vnd.ms-excel', 'xlsx' => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'doc' => 'application/msword', 'png' => 'image/png','jpg' => 'image/jpeg', 'sketch' => 'application/octet-stream', 'revit' => 'application/octet-stream',
	...
	function resize_with_ratio($file,$thumbnailname,$width,$height) {
   		
   		$file = $this->folderpath ."/".$file;
   		$file_ext = explode('.', $file);
   		$image_ext = ".".end($file_ext);	
	    $info = getimagesize($file);
	    $thumbnailname = $this->folderpath ."/".$thumbnailname;
	    ...
	    $scale_w = $width / $c_width;
	    $scale_h = $height / $c_height;
	    ...
	    imagecopyresampled($image, $image_old, $xpos, $ypos, 0, 0, $new_width, $new_height, $c_width, $c_height);
	    
	    if ($mime == 'image/png') {
	      imagepng($image, $thumbnailname.$image_ext);
	    } else {
	       imagejpeg($image, $thumbnailname.$image_ext,90);
	    }
