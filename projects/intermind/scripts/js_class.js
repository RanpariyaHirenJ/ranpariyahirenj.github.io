...
function jquery_ajax(func, url, params, element, loader) {
    if (typeof ($(loader)) != "undefined") {
        $(loader).css({
            "top": (window.innerHeight - $(loader).outerHeight()) / 2,
            "left": ((window.innerWidth - $(loader).outerWidth()) / 2)
        })
        $(".loader").fadeIn();
    }
    $.post(url, {
...
function initialize_lightbox(container, float) {
    ...
    if (float == "fixed") {
        ...
        $(container).css({
            "top": 50,
            "left": ((window.innerWidth - $(container).outerWidth()) / 2)
        })
    } else {
        $(container).css("position", float);
        $(container).css({
            "top": (window.innerHeight - $(container).outerHeight()) / 2,
            "left": ((window.innerWidth - $(container).outerWidth()) / 2)
        })
    }
