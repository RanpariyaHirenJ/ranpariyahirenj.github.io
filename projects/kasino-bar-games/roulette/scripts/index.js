...
function bgrotateTo(deg) {
    ...
    $(numbg).playKeyframe({
        ...
        complete: function () {
            var flag = false;
            var winner_name = "No winner found";

            $(".client-number").each(function () {
                if ($(this).html() == $("#numwin").val()) {
                    winner_name = $(this).parent().find('.client-name-lst').html()
                    flag = true;
                }
            })
            ...
            setTimeout(function () {
                if (flag == true) {
                    //$('.overlay').find('.overlay-box').addClass('animated').addClass(animationIn).addClass(animationOut);
                    //$(".roulette-cntr,.covercharge-cntr,.client-name,.countdown-cntr").fadeIn("700");
                    //$(".winner-dtls").append("Winner Name is : " + winner_name)
                    //$('body .overlay-bg,.overlay-box,.overlay').fadeIn("2500");                                                              

                    //$("#bgvid").fadeIn(2000);
                    //$(".main-container").css('display', 'none');
                    //$(".spin-btn").css('display', 'none');

                    //$('body .overlay-bg, .overlay-box, .overlay:eq(1)').fadeIn('2000').css('visibility', 'visible');
                    // $('body .overlay-bg, .overlay-box, .overlay:eq(1)').fadeIn("2000").css({ opacity: 0.0, visibility: "visible" }, 2000).animate({ opacity: 1.0 }, 2000);
                    //$('#winner').removeClass('fadeIn').addClass('fadeOut');
                    //$('body').css('background', 'none');
                    //$("#bgvid").fadeIn(2000).delay(500);

                    //$('.client-name').removeClass('fadeOut');
                    //$('.roulette-cntr, .covercharge-cntr').removeClass('fadeOutUp');

                    //$('#winner, #winner .overlay-box').css('opacity', '1').css('visibility', 'visible');

                    //$('#winner').addClass('fadeIn');
                    //$('#winner .overlay-box').addClass('fadeIn');                    

                    $("#bgvid").fadeIn(2500);
                    $('#winner, #winner .overlay-box').css({ opacity: 0.0, visibility: "visible" }, 3500).animate({ opacity: 1.0 }, 3500);
                    $(".main-container").css('display', 'none');
                    //$('.client-name').removeClass('fadeOut');
                    //$('.roulette-cntr, .covercharge-cntr').removeClass('fadeOutUp');

                    $('.roulette-cntr').removeClass('fadeOutUp').addClass('fadeInUp');
                    $('.covercharge-cntr').removeClass('fadeOutUp').addClass('fadeInUp');
                    $('.client-name').removeClass('fadeOut').removeClass('animated1').addClass('animated3').addClass('fadeIn');
                }
                else {
                    $('.overlay').find('.overlay-box').addClass('animated').addClass(animationIn).addClass(animationOut);
                    $(".roulette-cntr, .covercharge-cntr, .client-name, .countdown-cntr").fadeIn("700");
                    $('body .overlay-bg, .overlay-box, .overlay:eq(0)').fadeIn("2500");
                }
            }, 3500)
