...
function form_validate_jquery(container)
{
	...
	$(container).find("input, select, textarea, checkbox,  file , #program , #choose-event ,  #choose-radio" ).each(function(){
		...
		{				
			...
			case "table_checkbox":
				...
				if ($(this).find("input[type=checkbox]").is(":checked")) {
					...
					if($(this).find(':checkbox:checked').closest('td').attr('data-index')==3 && $(this).find(':checkbox:checked').length==1){
						$(this).css("border","2px solid green");	
					}
					else if($(this).find(':checkbox:checked').closest('td').attr('data-index')!=$(this).find(':checkbox:checked').length){
			...
			case "one-checkbox":
				...
				if ($(this).find("input[type=checkbox]").is(":checked")) {
						// $(this).css("border","2px solid green");
						$(this).find("input[type=checkbox]").removeAttr("style");
				}
				else {
					$(this).find("input[type=checkbox]").css({"background-color": "rgb(225, 0, 0)","-webkit-box-shadow": "0px 0px 4px 1px rgba(255,0,0,1)","-moz-box-shadow": "0px 0px 4px 1px rgba(255,0,0,1)","box-shadow": "0px 0px 4px 1px rgba(255,0,0,1)"});
					return_state = false;		
				}
			...
			case "one-radio":
				...
				if(!$(this).find("input[type=radio]").is(":checked")) {
					overlayBox('pop-radio');
					return_state = false;		
				}else{
					overlayBox('pop-nationality');
					if($(this).attr("permission")=="false"){
						$(this).attr("permission","true");
						return_state = false;		
					}
...
$("#program input[type=checkbox]").click(function(){

	if($(this).prop('checked')){
		if($(this).closest('td').attr('data-index')=="3"){

			$('td[data-index="2"],td[data-index="1"]').find(':checkbox').attr('disabled','disabled');

		}else if($(this).closest('td').attr('data-index')=="2"){

			if($(this).closest('td').find(':checkbox').filter(':checked').length==2){
				$(this).closest('td').find(':checkbox:not(:checked)').attr('disabled','disabled')
			}

			$('td[data-index="3"],td[data-index="1"]').find(':checkbox').attr('disabled','disabled');

		}else if($(this).closest('td').attr('data-index')=="1"){

			if($(this).closest('td').find(':checkbox').filter(':checked').length==1){
				$(this).closest('td').find(':checkbox:not(:checked)').attr('disabled','disabled')
			}

			$('td[data-index="3"],td[data-index="2"]').find(':checkbox').attr('disabled','disabled');
		}
	}else{

		if($(this).closest('td').attr('data-index')=="3"){

			$('td[data-index="2"],td[data-index="1"]').find(':checkbox').removeAttr('disabled');

		}else if($(this).closest('td').attr('data-index')=="2"){
			$(this).closest('td').find(':checkbox').removeAttr('disabled','disabled');

			if($(this).closest('td').find(':checkbox').filter(':checked').length==0){
				$('td[data-index="3"],td[data-index="1"]').find(':checkbox').removeAttr('disabled');
			}
			...
			if($(this).closest('td').find(':checkbox').filter(':checked').length==0){
				$('td[data-index="3"],td[data-index="2"]').find(':checkbox').removeAttr('disabled');
			}
...
$(".sign-in-register").click(function(){
	localStorage.setItem("active", 'NA');
});

$(".core-menu li").each(function(){
	if(localStorage.getItem("active")!="undefined" && localStorage.getItem("active")!=null && localStorage.getItem("active")!=""){
		if($(this).find('a').text() == localStorage.getItem("active")){
			$(this).find('a').addClass('active');
		}
	}else{
		$('#main-menu > div > nav > div > ul > li:nth-child(1) > a').addClass('active');
	}
