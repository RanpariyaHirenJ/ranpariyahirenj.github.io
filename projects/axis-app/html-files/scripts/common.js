...
//  $('body').append('<div id="empDtlError" class="overlay-box"><div class="seisson-message error"><span></span> Overlay id is not defined </div></div>');
  ...
  $('.input-group textarea, .input-group input,.input-group select').each(function () {
	...
	$(document).on('click', '.bottom-overlay h2', function () {
		$('.bottom-overlay').fadeIn();
		$('.bottom-overlay').addClass('open-nav');
	});
	$(document).on('click', '.bottom-overlay.open-nav h2', function () {
		$('.bottom-overlay').removeClass('open-nav');
	});
	
	$(window).load(function () {
  var liMaxHeight = -1;
    var node;
    $(".bottom-overlay .overlay-cntr").each(function(index) {
    ...
    $('.bottom-overlay').css('height', liMaxHeight + 38);
    $('.bottom-overlay').css('bottom', -liMaxHeight + 2);
   //alert(liMaxHeight)
})
	
	//var bottom_overlay_height = $(".bottom-overlay .overlay-cntr").height();
	//alert(bottom_overlay_height)
